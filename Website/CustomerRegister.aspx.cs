...
public partial class CustomerRegister : System.Web.UI.Page
{
    ...
    protected void SaveChangesClicked(object sender, EventArgs e)
    {
        try
        {
                ...
                if (itrig == 0)
                {
                    if (objAdm.CRUDCustomerUsers (rec_id.Value, RoleID.SelectedValue, fullname.Text, username.Text, objAdm.Encrypt("welcome"), email.Text, telephone.Text, "INSERT") == true)
                    {
                        LoadGrid();
                ...
                if (itrig == 1)
                {
                    if (objAdm.CRUDCustomerUsers(rec_id.Value, RoleID.SelectedValue, fullname.Text, username.Text, objAdm.Encrypt("welcome"), email.Text, telephone.Text, "UPDATE") == true)
                    {
                        LoadGrid();
                        DisplaySuccess(objAdm.ErrorMessage);
